...
- **No screenshot of the Stage 3 terminal session.** That stage is evidenced by its artefacts rather than by a capture of the CLI itself.
- **`deploy.ps1` is not exercised by the suite.** Group C reads the script and asserts it names all four runtime files; it does not run `Compress-Archive` and inspect the resulting archive, because the suite is written to run on any platform.
