...
- **No demo video.** The three screenshots in `docs/` show the start, middle and end states, not the accordion in motion.
...
- **`deploy.ps1` is not exercised by the suite, and the archive was never opened.** Group C reads the script and asserts it names all four runtime files; it does not run `Compress-Archive`, because the suite is written to run on any platform. The script was run by hand and reported success, but nobody has extracted `ecosip_deploy.zip` and confirmed all four files are inside. This is the weakest remaining link in the verification.
